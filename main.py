...
class run_detection:
    ...
    if display_lst == "d":
        ...
        min_detection_confidence = float(input("Minimum Detection Confidence (default = 0.5) - "))
        min_tracking_confidence = float(input("Minimum Tracking Confidence (default = 0.5) - "))

    def __init__(self):
        
        
        self.my_capture = cv2.VideoCapture(1) # camera index (sort of), used DroidCAM to model phone camera as webcam
        ...
        while True:
            ...
            if len(lmList) != 0: 
                ...
                cv2.circle(self.my_frames,(x1,y1),7,(163, 111, 103),cv2.FILLED)
                cv2.circle(self.my_frames,(x2,y2),7,(163, 111, 103),cv2.FILLED)
                cv2.line(self.my_frames, (x1,y1),(x2,y2),(89, 89, 255), 5)
                cv2.circle(self.my_frames,(cx,cy),6,(255, 222, 205),cv2.FILLED)
                
                #mapping length, as volume scale
                length = math.hypot(y2-y1, x2-x1)
                minVol = -50.0
                maxVol = 0.0
                vol = np.interp(length,[20,145],[minVol, maxVol])
                print(int(vol))
                #set volume function
